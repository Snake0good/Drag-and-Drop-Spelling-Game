...
// bees 
let beesLength = document.querySelectorAll('.flying-bee').length
const bees = []
for (let i=0; i<beesLength; i++) {
  bees[i] = new FlyingBee(`bee${i}`)
}

function randomSpeed(i) {
  bees[i].speed.x= 1000*(Math.random()-0.5);
  bees[i].speed.y= 1000*(Math.random()-0.5);
}
...
// Paint
// check for spelling bee page to be displayed and then paint them
console.log(introductionPage.style.display)
function startStopBees() {
  if (introductionPage.style.display === '') {
    console.log('running bees')
    window.setInterval(function(){
      for (let i=0; i<bees.length; i++) {
        bees[i].paint()
      }
